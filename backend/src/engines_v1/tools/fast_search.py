"""Fast search tool using Exa fast mode for quick queries."""
...
class FastSearchTool(BaseTool):
    """Fast search using Exa fast mode for standard queries.

    Optimized for speed with Exa's fast search mode.
    """
    ...
    def description(self) -> str:
        return "Fast search for standard queries using Exa fast mode - optimized for speed and relevance"
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "num_queries": {
                    "type": "integer",
                    "description": "Number of queries to use (3 recommended for standard mode)",
                    "minimum": 1,
                    "maximum": 5
                }
            },
    ...
    def _build_candidates_with_snake(
        ...
        """Build candidates using snake ordering - borrowed from parallel_search.
    ...
    async def execute(self, queries: List[str], num_queries: int = 3) -> Dict[str, Any]:
        """Execute fast search with Exa fast mode.
        ...
        try:
            ...
            return await self._search_with_exa_fast(queries)
    ...
    async def _search_with_exa_fast(self, queries: List[str]) -> Dict[str, Any]:
        """Search using Exa AUTO mode for speed."""
        ...
        # Wrap sources in XML structure with standard mode guidance
        if merged_sources:
            structured_highlights = f"""<sources>
{merged_sources}
</sources>

Based on the above sources, provide a comprehensive answer with proper citations [n]. Match the user's tone and context."""
        else:
